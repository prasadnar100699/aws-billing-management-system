...
class BillingMethodEnum(Enum):
    PER_UNIT = 'per_unit'
    TIERED = 'tiered'
    FLAT = 'flat'


class InvoicePreferenceEnum(Enum):
    MONTHLY = 'monthly'
    QUARTERLY = 'quarterly'
    CUSTOM = 'custom'


class ImportSourceEnum(Enum):
    CSV = 'CSV'
    API = 'API'
    ...
    def to_dict(self):
        return {
            ...
            'permissions': {
                ...
                for access in self.module_access
            }
        }
...
class User(UserMixin, db.Model):
    ...
    assigned_clients = db.relationship(
        'Client',
        secondary=user_client_mappings,
        lazy='subquery',
        backref=db.backref('assigned_managers', lazy=True)
    )
...
class Client(db.Model):
    ...
    usage_imports = db.relationship('UsageImport', backref='client', lazy=True)
    ...
    invoice_templates = db.relationship('InvoiceTemplate', backref='client', lazy=True)
...
class Invoice(db.Model):
    ...
    usd_to_inr_rate = db.Column(db.Numeric(10, 4))
    ...
    documents = db.relationship('Document', backref='invoice', lazy=True)
    ...
    def generate_invoice_number(self):
        ...
        if not self.invoice_number:
            ...
            last_invoice = Invoice.query.filter(
                ...
                Invoice.invoice_number.like(f'TejIT-{self.client_id}-{year_month}-%')
            ...
            self.invoice_number = f'TejIT-{self.client_id}-{year_month}-{sequence}'
    
    def calculate_totals(self):
        """Calculate invoice totals"""
        subtotal = sum(item.quantity * item.rate * (1 - item.discount/100) for item in self.line_items)
